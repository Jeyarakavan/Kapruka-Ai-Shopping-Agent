...
// Common product/category keywords for fast-path shopping detection
const SHOPPING_KEYWORDS = [
  'phone', 'laptop', 'computer', 'tablet', 'tv', 'television', 'camera', 'headphone',
  'watch', 'clock', 'shoe', 'shoes', 'shirt', 'dress', 'trouser', 'bag', 'handbag',
  'perfume', 'makeup', 'skincare', 'cream', 'lotion', 'shampoo', 'soap',
  'book', 'toy', 'game', 'board game', 'puzzle',
  'furniture', 'sofa', 'chair', 'table', 'bed', 'mattress', 'pillow', 'blanket', 'curtain',
  'kitchen', 'utensil', 'cookware', 'appliance', 'fridge', 'oven', 'microwave',
  'chocolate', 'cake', 'flower', 'flowers', 'bouquet', 'hamper',
  'ring', 'necklace', 'bracelet', 'jewellery', 'jewelry', 'earring',
  'sports', 'gym', 'fitness', 'cricket', 'football',
  'baby', 'infant', 'kids', 'children', 'toddler',
  'organic', 'herbal', 'ayurvedic',
];

function detectIntentFast(message, sessionContext = {}) {
  const lower = message.toLowerCase().trim();

  // ── HIGHEST PRIORITY: Active checkout session continuity ──
  // If we have checkout data in progress, keep routing there.
  const hasActiveCheckout = !!(
    sessionContext.checkoutData?.productId ||
    sessionContext.checkoutData?.recipientName ||
    sessionContext.cart?.length
  );

  if (hasActiveCheckout) {
    // Affirmative replies (yes, proceed, ok, confirm, sure, etc.)
    if (/^(yes|yeah|yep|yup|ok|okay|sure|proceed|confirm|go ahead|do it|create|place|make it|finalize|complete|correct|right|sounds good|that.?s (right|correct|good|fine)|please proceed|go|continue|checkout now)/i.test(lower)) {
      return 'checkout';
    }
    // City / address updates during checkout
    if (
      /^(jaffna|colombo|kandy|galle|matara|negombo|kurunegala|ratnapura|badulla|trincomalee|batticaloa|anuradhapura|polonnaruwa|vavuniya|mannar|kilinochchi|mullaitivu|chilaw|puttalam|kegalle|nuwara|kalutara|gampaha|hambantota|ampara|monaragala|matale|dambulla|sigiriya|tangalle)/i.test(lower)
    ) {
      return 'checkout';
    }
    // Number-only input during checkout (e.g. selecting an option, entering quantity)
    if (/^\d+$/.test(lower)) {
      return 'checkout';
    }
    // Explicitly adding info (name, phone, address, date)
    if (/my name is|name:|phone|address:|delivery date|sender|gift message|message for/i.test(lower)) {
      return 'checkout';
    }
  }

  // ── awaitingConfirmation from last response ──
  if (sessionContext.awaitingConfirmation) {
    if (/^(yes|yeah|yep|yup|ok|okay|sure|proceed|confirm|go ahead|do it|create|place|finalize|complete|correct|right|sounds good|please proceed|go|continue)/i.test(lower)) {
      return 'checkout';
    }
  }
  ...
  if (/track (my )?order|order status|where is my (package|order|delivery)/i.test(lower)) return 'tracking';
  ...
  // Fast-path shopping: explicit product search patterns
  if (/^(show me|find|search|buy|get|need|want|looking for|i need|i want)\s+/i.test(lower)) {
    // Exclude if it's clearly a gift/person context
    if (!/gift|girlfriend|boyfriend|wife|husband|friend|sister|brother|mom|dad/i.test(lower)) {
      return 'shopping';
    }
  }

  // Fast-path shopping: known product categories
  if (SHOPPING_KEYWORDS.some((kw) => lower.includes(kw))) {
    // Only if not a gift context
    if (!/(gift|for my|for a|for her|for him|girlfriend|boyfriend|anniversary|birthday)/i.test(lower)) {
      return 'shopping';
    }
  }
  ...
  try {
    switch (intent) {
      ...
      case 'checkout': {
        ...
        // Detect if this is a confirmation reply
        const lower = userMessage.toLowerCase().trim();
        const isConfirmation =
          sessionContext.awaitingConfirmation ||
          sessionContext.isConfirmation ||
          /^(yes|yeah|yep|yup|ok|okay|sure|proceed|confirm|go ahead|do it|create|place|finalize|complete|correct|right|sounds good|please proceed|go|continue|checkout now)/i.test(lower);

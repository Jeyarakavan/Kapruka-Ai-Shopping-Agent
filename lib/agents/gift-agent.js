...
import { filterProductsByPrice } from '../shopping/intent-parser.js';
import {
...
async function searchGiftProducts(ctx, occasion) {
  ...
  const searches = [keyword, 'flowers gift', 'chocolate gift', 'jewellery gift'];
...
export async function runGiftAgent(userMessage, conversationHistory = [], sessionContext = {}) {
  ...
  const wantsProducts =
    isShowProductsRequest(userMessage) || hasEnoughGiftContext(mergedContext);

  if (!wantsProducts || !hasEnoughGiftContext(mergedContext)) {
